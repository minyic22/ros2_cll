...
class ShapeLearner:
    def __init__(self, settings):
        ...
        for i in range(len(self.paramsToVary)):
            ...
            if (np.isnan(settings.initialBounds[i, 0]) or np.isnan(
                    settings.initialBounds[i, 1])):  #want to set initial bounds as std. dev. multiple
                boundsFromStdDevMultiples = np.array(settings.initialBounds_stdDevMultiples[i, :]) * \
                                            parameterVariances[settings.paramsToVary[i] - 1]

                if (np.isnan(settings.initialBounds[i, 0])):
                    self.bounds[i, 0] = boundsFromStdDevMultiples[0]
                if (np.isnan(settings.initialBounds[i, 1])):
        ...
        self.params = np.zeros((self.numPrincipleComponents, 1))
    ...
    def startLearning(self):
        #make initial shape
        if (np.isnan(self.initialParamValue[0])):
    ...
    def generateSimulatedFeedback(self, shape, newParamValue):
        ...
        goalParamValue = np.float64(0) #-1.5*parameterVariances[self.paramToVary-1]
        goalParamsValue = np.zeros((self.numPrincipleComponents, 1))
        goalParamsValue[self.paramsToVary - 1, 0] = goalParamValue
        if (self.doGroupwiseComparison):
            errors = np.ndarray.tolist(abs(self.shapeToParamsMapping - goalParamsValue))
    ...
    def respondToFeedback(self, bestShape):
        ...
        if (self.doGroupwiseComparison):
            ...
            diff = np.linalg.norm(diff_params)
    ...
    def respondToDemonstration(self, shape):
        ...
        """
        demo_shape = ShapeModeler.normaliseShapeHeight(np.array(shape))
...
if __name__ == '__main__':
    ...
    init_datasetFile = 'a.dat'
    update_datasetFile = 'a.dat'
    demo_datasetFile = 'a.dat'

    try:
        datasetParam = 'params.dat'
